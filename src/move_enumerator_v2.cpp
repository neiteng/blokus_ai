...
MoveEnumeratorV2::MoveEnumeratorV2(const Field& originalField, const ConstPlayerPtrList& playerPtrList, std::shared_ptr<const PieceAnalyst> pieceAnalystPtr_)
...
void MoveEnumeratorV2::add(int playerId, const MoveCand& moveCand)
{
...
void MoveEnumeratorV2::listMove(int playerId, int x, int y, int defId, int varId)
{
...
void MoveEnumeratorV2::cleanUpMoveInfoList()
{
	for(int playerId = 0; playerId < PLAYER_NUM; playerId++)
	{
		...
		/*
		for(int index = 0; index < moveInfoList[playerId].size(); index++)
		{
			if(moveInfoList[playerId][index]->isValid) continue;
			moveInfoList[playerId][index] = moveInfoList[playerId].back();
			moveInfoList[playerId].pop_back();
		}
		*/
	}
}

int MoveEnumeratorV2::getMoveNum(int playerId) const
{
	return moveInfoList[playerId].size();
}

const MoveCand& MoveEnumeratorV2::getMove(int playerId, int moveId) const
{
	return moveInfoList[playerId][moveId]->move;
}

bool MoveEnumeratorV2::isUsed(int playerId, int defId) const
{
	return defTable[playerId][defId].isUsed;
}

void MoveEnumeratorV2::init()
{
	//初期状態を復元
	for(int playerId = 0; playerId < PLAYER_NUM; playerId++)
	{
